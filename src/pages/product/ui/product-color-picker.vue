...
function getOptionColor(product: StoreProduct | SearchProductDocument) {
  return product.options
    ?.find((option) => option.title?.toLowerCase() === "color")
    ?.values?.[0]?.value;
}

function getDisplayColor(product: StoreProduct | SearchProductDocument) {
  return (
    (product.variants?.[0]?.metadata?.colorTranslation as string | undefined) ??
...
function getProductImage(product: StoreProduct | SearchProductDocument) {
  ...
  const imageByColor = product.images?.find(
    (image) => normalizeValue(image.metadata?.color as string | undefined) === variantColor,
  );
...
const currentColorKey = computed(() => normalizeValue(getDisplayColor(product.value)));
...
const { data: siblingProductsResponse } = await useAsyncData(
  ...
  async () => {
    ...
    const response = await searchClient.index("cards").search<SearchProductDocument>(null, {
      filter: [`metadata.model = "${escapeFilterValue(currentModel.value)}"`],
      distinct: "id",
      hitsPerPage: 100,
    });
...
const colorItems = computed(() => {
  ...
  for (const item of source) {
    const key = normalizeValue(getDisplayColor(item)) || item.handle || item.id;
    if (!deduped.has(key)) {
...
function goToProductColor(targetProduct: SearchProductDocument) {
  if (!targetProduct.handle || targetProduct.handle === product.value?.handle) {
    return;
  }
  ...
  <div class="flex my-4 lg:my-9 flex-col gap-5 lg:gap-4">
    <span>
      ...
      <span class="capitalize">
        {{ variant?.metadata?.colorTranslation ?? color?.value ?? getDisplayColor(product) }}
      </span>
    </span>
    <div class="flex overflow-x-auto gap-3 w-full">
      <article
        ...
        :class="{
          'border-blue': normalizeValue(getDisplayColor(item)) === currentColorKey,
          'border-transparent': normalizeValue(getDisplayColor(item)) !== currentColorKey,
        }"
      >
        <img
